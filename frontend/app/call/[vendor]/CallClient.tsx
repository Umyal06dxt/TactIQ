...
type Phase = "idle" | "connecting" | "loading" | "live" | "demo" | "ended" | "saving";
    ...
    <div className="flex flex-col items-center gap-1">
      <div className="relative w-16 h-16">
        <svg className="w-16 h-16 -rotate-90" viewBox="0 0 64 64">
          <circle cx="32" cy="32" r={r} strokeWidth="5" className="stroke-white/8 fill-none" />
          <circle
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-sm font-black" style={{ color }}>{prob}%</span>
        </div>
      </div>
...
function MomentumBadge({ momentum }: { momentum: "positive" | "neutral" | "negative" }) {
  const styles = {
    positive: { cls: "bg-emerald-500/15 border-emerald-500/30 text-emerald-400", icon: "↑", label: "Positive" },
    neutral: { cls: "bg-white/8 border-white/15 text-white/40", icon: "→", label: "Neutral" },
    negative: { cls: "bg-red-500/15 border-red-500/30 text-red-400", icon: "↓", label: "Negative" },
  };
  const s = styles[momentum];
  return (
    <span className={`inline-flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded-full border uppercase tracking-wider ${s.cls}`}>
      {s.icon} {s.label}
    </span>
  );
}
...
export function CallClient({ vendor, briefing }: { vendor: string; briefing: Briefing }) {
  ...
  const [coaching, setCoaching] = useState<CoachUpdate>({
    suggestions: [], warnings: [], correction: null, current_tactic: null,
    win_probability: 50, concession_alert: null, deal_momentum: "neutral",
    coach_note: null, concession_count: 0,
  });
  ...
  const pushConcessionAlert = useCallback((text: string) => {
    ...
    concessionTimerRef.current = setTimeout(() => setConcessionAlert(null), 8000);
  }, []);
  ...
  const startRecording = useCallback((stream: MediaStream, ws: WebSocket) => {
    ...
    const segment = () => {
      ...
      recorder.onstop = async () => {
        ...
        if (blob.size < 1500) { segment(); return; }
        try {
  ...
  }, [vendor, duration, transcript, coachingHistory, pendingOutcome, briefing, coaching.concession_count]);
    ...
    <div className="h-screen bg-[#05080f] text-white flex flex-col overflow-hidden">
      ...
      {concessionAlert && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
          <div className="bg-red-500 text-white rounded-2xl px-5 py-3 shadow-2xl shadow-red-500/30 flex items-start gap-3 max-w-sm animate-[slideDown_0.25s_ease-out]">
            <span className="text-xl mt-0.5">🚨</span>
            <div className="flex-1">
              <div className="font-black text-xs uppercase tracking-widest mb-1">Concession Alert</div>
              <div className="text-sm font-medium leading-snug">{concessionAlert}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="shrink-0 border-b border-white/8 px-6 py-3 flex items-center justify-between gap-6 bg-black/20">
        <div className="flex items-center gap-4 min-w-0">
          <a href={`/briefing/${vendor}`} className="shrink-0 text-white/30 hover:text-white/60 transition text-sm">
            ← Briefing
          </a>
        ...
        <div className="flex items-center gap-4 shrink-0">
          {isLive && coaching.concession_count > 0 && (
            <div className="flex items-center gap-1.5 bg-red-500/10 border border-red-500/20 text-red-400 text-[10px] font-bold px-2.5 py-1 rounded-full">
              <span>⚡</span>
              {coaching.concession_count} concession{coaching.concession_count !== 1 ? "s" : ""}
            </div>
          )}
          {wsError && <span className="text-xs text-red-400 max-w-xs truncate">{wsError}</span>}
          ...
          {phase !== "idle" && (
            <span className={`text-xs px-2.5 py-1 rounded-full font-bold tracking-wider ${
              ...
              phase === "demo" ? "bg-blue-500/15 text-blue-400 border border-blue-500/30" :
              ...
              {phase === "live" ? "LIVE" : phase === "demo" ? "DEMO" : phase === "connecting" ? "CONNECTING" : phase === "loading" ? "LOADING" : phase === "saving" ? "SAVING" : "ENDED"}
            </span>
          )}
        </div>
      </header>
        ...
        <div className="border-r border-white/8 flex flex-col bg-emerald-950/10">
          <div className="px-6 pt-5 pb-3 border-b border-white/5 flex items-center gap-2">
            ...
            {isLive && !coachLoading && coaching.suggestions.length > 0 && (
              <span className="ml-auto text-[9px] text-emerald-900 font-mono">1·2·3 to copy</span>
            )}
            {coachLoading && isLive && (
              ...
              <>
                    ...
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-emerald-600 font-bold uppercase tracking-wider">
                          {i === 0 ? "Recommended" : i === 1 ? "Alternative" : "Deflect"}
                        </span>
                      </div>
                ...
                {coaching.coach_note && (
                  <div className="bg-blue-950/30 border border-blue-800/30 rounded-xl p-3 mt-2">
                    <p className="text-[10px] text-blue-400 font-bold uppercase tracking-wider mb-1">Coach insight</p>
                    <p className="text-xs text-blue-300/70 leading-relaxed italic">{coaching.coach_note}</p>
                  </div>
                )}
              </>
            )}
          ...
          {isLive && (
            <div className="w-full flex items-center justify-center gap-4 bg-white/3 rounded-xl border border-white/8 py-3">
              ...
              <div className="text-left">
                ...
                <p className="text-2xl font-black text-white">{coaching.win_probability}</p>
          ...
          {phase === "idle" && (
            <div className="flex flex-col items-center gap-5 text-center">
              ...
              <p className="text-xs text-white/20 max-w-[180px] leading-relaxed">
                Watch a scripted negotiation with real AI coaching — no mic needed
              </p>
            </div>
          )}
              ...
              <div className="text-center">
                {phase === "demo" ? (
                  <p className="text-blue-400/70 text-xs flex items-center justify-center gap-2">
                    ...
                    Demo playing · {transcript.length}/{DEMO_SCRIPT.length} turns · {tacticsDeployed} tactics
                  </p>
                ) : (
                  <p className="text-white/50 text-xs flex items-center justify-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse inline-block" />
                    Mic active · {transcript.length} turns · {tacticsDeployed} tactics
                  </p>
                )}
              </div>
                  ...
                  {(["won", "lost", "pending", "escalated"] as const).map((o) => (
                    <button
                      ...
                      className={`py-1.5 rounded-lg text-xs font-semibold capitalize transition-all ${
                        pendingOutcome === o
                          ? o === "won" ? "bg-emerald-500 text-black"
                          : o === "lost" ? "bg-red-500 text-white"
                          : o === "pending" ? "bg-yellow-500 text-black"
