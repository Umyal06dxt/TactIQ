...
const MOMENT_DEFAULT = {
  style: "border-white/10 bg-white/5",
  icon: "bg-white/8 text-white/30 border border-white/10",
};
  ...
  return (
    <div className="flex items-center gap-6">
      ...
      <div>
        ...
        <p
          className={`text-sm font-bold mt-0.5 ${score >= 70 ? "text-emerald-400" : score >= 40 ? "text-yellow-400" : "text-red-400"}`}
        >
          {score >= 70
            ? "Strong execution"
            : score >= 40
              ? "Moderate — room to improve"
              : "Needs work — review key moments"}
        </p>
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch(
        `${BASE}/api/calls/${vendor}/${callId}/follow-up`,
        { method: "POST" },
      );
    ...
    <section className="border-t border-white/8 pt-6">
      ...
      {state === "idle" && (
        <p className="text-xs text-white/20 italic">
          AI-drafted follow-up email based on this call's outcome and key
          moments.
        </p>
      )}
      ...
      {state === "done" && email && (
        <div className="rounded-xl border border-white/10 bg-white/3 p-4 space-y-3">
          ...
          <div>
            ...
            <pre className="text-xs text-white/55 leading-relaxed whitespace-pre-wrap font-sans">
              {email.body}
            </pre>
          </div>
        </div>
...
export function CallSummaryDetail({ call }: { call: CallRecord }) {
  ...
  const adherenceScore =
    call.adherence_score != null ? call.adherence_score * 100 : null;
  const winProbPct =
    call.win_probability != null
      ? Math.round(call.win_probability * 100)
      : null;
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4 pb-6 border-b border-white/8">
        ...
        <div className="flex flex-col items-end gap-2">
          {call.outcome && (
            <span
              className={`shrink-0 text-sm font-bold px-3 py-1.5 rounded-full capitalize ${OUTCOME_STYLES[call.outcome] ?? "bg-white/8 text-white/30 border border-white/10"}`}
            >
              {call.outcome}
            </span>
          )}
        ...
        <section className="grid grid-cols-3 gap-4">
          ...
          {winProbPct != null && (
            <div className="bg-white/3 border border-white/8 rounded-2xl p-4 flex flex-col items-center justify-center gap-1 text-center">
              <p
                className={`text-3xl font-black ${winProbPct >= 60 ? "text-emerald-400" : winProbPct >= 40 ? "text-yellow-400" : "text-red-400"}`}
              >
                {winProbPct}%
              </p>
              <p className="text-[10px] text-white/30 uppercase tracking-wider">
              ...
              <p
                className={`text-xs font-semibold ${winProbPct >= 60 ? "text-emerald-400" : winProbPct >= 40 ? "text-yellow-400" : "text-red-400"}`}
              >
                {winProbPct >= 60
                  ? "Likely won"
                  : winProbPct >= 40
          ...
          {call.concessions_made != null && (
            <div className="bg-white/3 border border-white/8 rounded-2xl p-4 flex flex-col items-center justify-center gap-1 text-center">
              <p
                className={`text-3xl font-black ${call.concessions_made === 0 ? "text-emerald-400" : call.concessions_made <= 2 ? "text-yellow-400" : "text-red-400"}`}
              >
              ...
              <p
                className={`text-xs font-semibold ${call.concessions_made === 0 ? "text-emerald-400" : "text-orange-400"}`}
              >
                {call.concessions_made === 0
                  ? "Zero unearned"
                  : "unearned moves"}
              </p>
            </div>
        ...
        <section className="border-t border-white/8 pt-6">
          ...
          <div className="relative space-y-3 pl-8">
            ...
            {call.key_moments.map((km: KeyMoment, i: number) => (
              <div
                key={i}
                className={`relative rounded-xl border p-4 ${MOMENT_STYLES[km.type] ?? MOMENT_DEFAULT.style}`}
              >
                <div
                  className={`absolute -left-5 top-4 w-5 h-5 rounded-full text-[10px] font-black flex items-center justify-center ${MOMENT_ICON_STYLES[km.type] ?? MOMENT_DEFAULT.icon}`}
                >
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="shrink-0 text-[10px] text-white/20 font-mono mt-0.5">
                    T{km.turn_index}
                  </span>
                </div>
              </div>
